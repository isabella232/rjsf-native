...
const _getMatchingOption = ({
	...
	validator: (schema: JSONSchema7, data: any) => boolean | PromiseLike<boolean>;
}): number => {
	const option = getMatchingOption(validator, formData, options, rootSchema);
	if (option !== 0) {
		return option;
	}
...
const AnyOfField = ({
	...
	const [selectedOptionIndex, setSelectedOptionIndex] = useState(
		_getMatchingOption({
			...
			validator: registry.schemaUtils.validator,
		})
	);
	...
	useEffect(() => {
		...
		options,
		rootSchema,
		registry.schemaUtils.validator,
	]);
